...
import { deleteFromCart } from "@/app/cartAndWislistSlice";
import { toast } from "react-toastify";

const ShoppingCart = () => {
  const { cart } = useSelector((state) => state.cartWishlist);
  const dispatch = useDispatch();

  if (cart.length === 0) {
    toast.warn("Looks like your cart is empty 🥲🥲");
  }

  const discountCalculater = (price, offerPrice) => {
  ...
  const checkOutDataHelper = () => {
    ...
    cart.forEach((prod) => {
      ...
      if (prod?.offerPrice > 0) {
        discount +=
          prod?.price * prod?.quantity - prod?.offerPrice * prod?.quantity;
      }
    });
